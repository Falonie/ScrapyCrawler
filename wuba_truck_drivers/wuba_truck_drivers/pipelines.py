...
class WubaTruckDriversPipeline(object):
    ...
    def process_item(self, item, spider):
        sql = "insert into wuba_truck_drivers2 (TITLE,POSITION,RECRUIT,LOCATION,COMPANY,INDUSTRY,SCALE," \
              "JOB_DESCRIPTION,COMPANY_DESCRIPTION) values('{}','{}','{}','{}','{}','{}','{}','{}','{}')"
        self.cursor.execute(sql.format(item['title'],item['position'],item['recruit'],item['location'],item['company'],
                            item['industry'],item['scale'],item['job_description'],item['company_description']))
        self.connection.commit()

        with open('wuba_truck_drivers.csv', 'a+', newline='') as f2:
