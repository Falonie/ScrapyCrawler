...
class ItjuziSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        #     yield {'link': amount}
        for product, link, amount,investor in zip(products, links, financial_amount,investors):
            # yield {'product': product, 'link': link, 'amount': amount}
            # product_name = item.xpath('span/text()').extract()
            p = {'product': product, 'financial amount': amount, 'investor': investor}
    ...
    def page(self,response):
        ...
        juzi['homepage'] = response.xpath(
            '//div[@class="picinfo"]/div[@class="link-line"]/a[@class="weblink"]/@href').extract()
        industry = [str(i) for i in response.xpath('//div[@class="picinfo"]/div[3]/span[1]/a/text()').extract()]
        juzi['industry'] = ''.join(industry)
        juzi['scale'] = str(response.xpath('//div[@class="des-more"]/div[2]/h2[2]/text()').extract()[0]).strip()
